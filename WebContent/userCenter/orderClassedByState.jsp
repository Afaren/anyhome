...
<title>order sort of state</title>
<style type="text/css">
	.navigation
	{
		margin-left: 20px;
...
<body>
	<div class = "navigation">
		...
		<li><a href="orderClassedByState?state=<%=OrderStates.CHECKING_ORDER%>&type=host&id=${sessionScope.loginedUser.user_id}" target="showFrame">待确认</a></li>
		<li><a href="orderClassedByState?state=<%=OrderStates.ACCEPT_WAIT_PAY_ORDER%>&type=host&id=${sessionScope.loginedUser.user_id}" target="showFrame">待付款</a></li>
		<li><a href="orderClassedByState?state=<%=OrderStates.WAIT_CHECKIN_ORDER%>&type=host&id=${sessionScope.loginedUser.user_id}" target="showFrame">待入住</a></li>
		<li><a href="orderClassedByState?state=<%=OrderStates.ACCOMPLISH_ORDER%>&type=host&id=${sessionScope.loginedUser.user_id}" target="showFrame">已完成</a></li>
		<li><a href="orderClassedByState?state=<%=OrderStates.CANCEL_ORDER%>&type=host&id=${sessionScope.loginedUser.user_id}" target="showFrame">已取消</a></li>
		<li><a href="orderClassedByState?state=<%=OrderStates.ACCOMPLISH_ORDER%>&type=host&id=${sessionScope.loginedUser.user_id}" target="showFrame">已过期</a></li>
	</div><br/><br/><br/>
	<%-- <%
